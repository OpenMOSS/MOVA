...
class Cnn14(nn.Module):
    ...
    def __init__(
        ...
        #               (home_dir, "https://zenodo.org/record/3576403/files/Cnn14_mAP%3D0.431.pth"))
        #     os.system(
        #         "wget -P %s/.cache/audioldm_eval/ckpt/ %s" %
        #         (home_dir, "https://zenodo.org/record/3987831/files/Cnn14_16k_mAP%3D0.438.pth"))
        ...
        # Resolve checkpoint paths: local pann_home first, then HF cache, then download
        _script_dir = os.path.dirname(os.path.abspath(__file__))
        _local_pann_dir = os.path.join(_script_dir, '..', '..', 'pann_home')
        _local_cnn14_32k = os.path.join(_local_pann_dir, "Cnn14_mAP=0.431.pth")
        _local_cnn14_16k = os.path.join(_local_pann_dir, "Cnn14_16k_mAP=0.438.pth")
        # Also check audio_is_clap directory for 32k checkpoint
        if not os.path.exists(_local_cnn14_32k):
            _alt_pann_dir = os.path.join(_script_dir, '..', '..', '..', 'audio_is_clap', 'pann_home')
            _alt_cnn14 = os.path.join(_alt_pann_dir, "Cnn14_mAP=0.431.pth")
            if os.path.exists(_alt_cnn14):
                _local_cnn14_32k = _alt_cnn14

        hf_home = os.environ.get("HF_HOME", os.path.expanduser("~"))
        ckpt_dir = os.path.join(hf_home, "audioldm_eval", "ckpt")

        # Find checkpoint for requested sample_rate
        if sample_rate == 16000:
            # 16k checkpoint: local pann_home → HF cache → download
            ckpt_path = None
            if os.path.exists(_local_cnn14_16k):
                ckpt_path = _local_cnn14_16k
            else:
                cached_16k = os.path.join(ckpt_dir, "Cnn14_16k_mAP=0.438.pth")
                if os.path.exists(cached_16k):
                    ckpt_path = cached_16k
            if ckpt_path is None:
                print("Download Cnn14_16k checkpoint.")
                os.makedirs(ckpt_dir, exist_ok=True)
                os.system(f"wget -P {ckpt_dir} https://zenodo.org/record/3987831/files/Cnn14_16k_mAP%3D0.438.pth")
                ckpt_path = os.path.join(ckpt_dir, "Cnn14_16k_mAP=0.438.pth")
            print(f"Loading Cnn14 (16k) from {ckpt_path}")
            state_dict = torch.load(ckpt_path, map_location="cpu", weights_only=False)
            self.load_state_dict(state_dict["model"])

        elif sample_rate == 32000:
            # 32k checkpoint: local pann_home → HF cache → download
            ckpt_path = None
            if os.path.exists(_local_cnn14_32k):
                ckpt_path = _local_cnn14_32k
            else:
                cached_32k = os.path.join(ckpt_dir, "Cnn14_mAP=0.431.pth")
                if os.path.exists(cached_32k):
                    ckpt_path = cached_32k
            if ckpt_path is None:
                print("Download Cnn14 (32k) checkpoint.")
                os.makedirs(ckpt_dir, exist_ok=True)
                os.system(f"wget -P {ckpt_dir} https://zenodo.org/record/3576403/files/Cnn14_mAP%3D0.431.pth")
                ckpt_path = os.path.join(ckpt_dir, "Cnn14_mAP=0.431.pth")
            print(f"Loading Cnn14 (32k) from {ckpt_path}")
            state_dict = torch.load(ckpt_path, map_location="cpu", weights_only=False)
            self.load_state_dict(state_dict["model"])
