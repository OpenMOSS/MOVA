...
BASE_DIR = os.environ.get("HF_HOME", os.path.join(os.path.expanduser("~"), ".cache", "huggingface"))
# HF cache stores models under hub/ subdirectory; check both locations
if not os.path.isdir(os.path.join(BASE_DIR, "models--roberta-base")) and os.path.isdir(os.path.join(BASE_DIR, "hub", "models--roberta-base")):
    BASE_DIR = os.path.join(BASE_DIR, "hub")

# Local model directories within the repo (relative to this file)
_LOCAL_MODELS_ROOT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', '..', '..', 'models')

def get_snapshot_path(model_dir):
    # First, check local models directory in the repo
    if model_dir == "models--roberta-base":
        local_path = os.path.join(_LOCAL_MODELS_ROOT, "roberta-base")
    ...
    raise FileNotFoundError(f"Model {model_dir} not found in local paths or HF cache")

# bert_tokenizer = BertTokenizer.from_pretrained(get_snapshot_path("models--bert-base-uncased"), local_files_only=True)
# roberta_tokenizer = RobertaTokenizer.from_pretrained()
# bart_tokenizer = BartTokenizer.from_pretrained(get_snapshot_path("models--facebook--bart-base"), local_files_only=True)
...
class CLAP_Module(torch.nn.Module):
    def __init__(self, enable_fusion=False, device=None, amodel= 'HTSAT-tiny', tmodel='roberta') -> None:
        ...
        self.tokenize = RobertaTokenizer.from_pretrained(get_snapshot_path("models--roberta-base"), local_files_only=True)
